...
const TypedArrayHelper = function (size, registers, register_type, array_type, unit_size, accessors) {
...
TypedArrayHelper.prototype = {
  ...
  index_to_register(index, register, isLoop) {
    const base = index * this.unit_size;
  ...
  from_existing(oldArray) {
    const new_size = oldArray.length;
  ...
  push_element(vector) {
    ...
    const bpos = this.length * this.unit_size;
...
function convertGeometryToIndexedBuffer(geometry) {
  ...
  const vertArray = new TypedArrayHelper(geometry.vertices.length, 0, THREE.Vector3, Float32Array, 3, ['x', 'y', 'z']);
  const indexArray = new TypedArrayHelper(geometry.faces.length, 0, THREE.Face3, Uint32Array, 3, ['a', 'b', 'c']);
  const uvArray = new TypedArrayHelper(geometry.faceVertexUvs[0].length * 3 * 3, 0, THREE.Vector2, Float32Array, 2, ['x', 'y']);
...
function compute_vertex_normals(geometry) {
  ...
  const oldVertices = new TypedArrayHelper(0, 5, THREE.Vector3, Float32Array, 3, XYZ);
  const oldFaces = new TypedArrayHelper(0, 3, THREE.Face3, Uint32Array, 3, ABC);
  oldVertices.from_existing(geometry.getAttribute('position').array);
  const newNormals = new TypedArrayHelper(oldVertices.length * 3, 4, THREE.Vector3, Float32Array, 3, XYZ);
  const newNormalFaces = new TypedArrayHelper(oldVertices.length, 1, function () { this.x = 0; }, Float32Array, 1, ['x']);
  ...
  let a,
    ...
    c;
  let my_weight;
  const full_weights = [0.0, 0.0, 0.0];
...
function unIndexIndexedGeometry(geometry) {
  ...
  const oldVertices = new TypedArrayHelper(0, 3, THREE.Vector3, Float32Array, 3, XYZ);
  const oldFaces = new TypedArrayHelper(0, 3, THREE.Face3, Uint32Array, 3, ABC);
  const oldUvs = new TypedArrayHelper(0, 3, THREE.Vector2, Float32Array, 2, XY);
  const oldNormals = new TypedArrayHelper(0, 3, THREE.Vector3, Float32Array, 3, XYZ);
  ...
  const newVertices = new TypedArrayHelper(oldFaces.length * 3, 3, THREE.Vector3, Float32Array, 3, XYZ);
  const newNormals = new TypedArrayHelper(oldFaces.length * 3, 3, THREE.Vector3, Float32Array, 3, XYZ);
  const newUvs = new TypedArrayHelper(oldFaces.length * 3, 3, THREE.Vector2, Float32Array, 2, XY);
  let v,
    w;
...
(function () {
  ...
  THREE.BufferSubdivisionModifier.prototype.smooth = function (geometry) {
    ...
    let other,
      ...
      newEdge,
      face;
    let edgeVertexWeight,
      adjacentVertexWeight,
      connectedFaces;

    const tmp = newVertices.register[1];
    ...
    const edgeLength = newVertices.length;
    /** ****************************************************
		*
		*	Step 2.
    ...
    const x3 = newUVs.register[0];
    const x4 = newUVs.register[1];
    const x5 = newUVs.register[2];
    const tFace = newFaces.register[0];
    ...
    for (i = 0, il = oldFaces.length; i < il; i++) {
      ...
      if (doUvs === true) {
        ...
        const x0 = oldUvs.register[0]; // uv[0];
        const x1 = oldUvs.register[1]; // uv[1];
        const x2 = oldUvs.register[2]; // uv[2];
